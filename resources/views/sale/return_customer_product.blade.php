...
@push('scripts')
    ...
    <script>
        ...
        function loadTreatmentData(sale_id) {
        ...
        @php
            $returnDetailBaseUrl = preg_replace('/\/0$/', '', route('pos.get_sale_detail_for_return', ['sdid' => 0]));
        @endphp
        var returnDetailBaseUrl = @json($returnDetailBaseUrl);
        ...
        $("body").on("click", ".return_product", function(e) {
            ...
            $.ajax({
                type: 'GET',
                url: returnDetailBaseUrl + '/' + sdid,
                dataType: 'json',
                success: function(res) {
                    if (!res.success || !res.data) {
                        alert(res.message || 'Failed to load item details.');
                        return;
                    }

                    var details = res.data;
                    discount_percentage = details.discount_percentage;
                    sale_price = details.UnitePrice;
                    ...
                    $("#Quantity").val(details.available_quantity);
                    $("#ReturnQuantity").val('');
                    $("#return_details").html('');
                },
                error: function(xhr) {
        ...
        $("body").on("click", "#return_item_to_stock", function(e) {
           var SDID= $("#SDID").val();
            ...
            $.ajax({
                type: 'post',
                url: "{{ route('pos.return_customer_parmacy_item') }}",
                data: {
                    SDID: SDID,

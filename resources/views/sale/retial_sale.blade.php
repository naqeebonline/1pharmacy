...
<body>
    ...
    <script type="text/javascript">
        ...
        function buildPosGridCellInput(field, value, productId) {
            ...
            var displayValue = (value === undefined || value === null || value === '') ? 0 : value;
        ...
        function applyPosGridCellChange($input) {
            ...
            if (quantity > avaliable_qty) {
                if (product) {
                    var packDisplay = packQtyDisplayFromUnitQty(product.Quantity, packSize);
                    $tr.find('[data-field="pack_qty"]').val(packDisplay);
                    $tr.find('[data-field="quantity"]').val(toIntQty(product.Quantity));
                } else {
                    $input.val(previousValue !== undefined ? previousValue : '');
                }
            ...
            updateProductByID(product_id, quantity, rate, total, pack_qty, true);
            posGridInputPrevious[inputKey] = $input.val();
        }
    ...
    function updateProductByID(product_id, quantity, rate, total, pack_qty = null, skipReload) {
        ...
        function refreshProductRowInGrid(productId) {
            ...
            var unitQty = toIntQty(product.Quantity);
            ...
            $tr.find('td:eq(5)').text(product.Total);
        ...
        function syncBillTotalsFromProductList() {
            ...
            ProductList.forEach(function(product) {
                calculateRowTaxAmounts(product);
                total_amount += parseFloat(product.Total) || 0;
            ...
            var netBeforeTax = Math.ceil(total_amount - total_product_discount_amount - parseFloat(invoice_discount));
            if (netBeforeTax < 0) {
                netBeforeTax = 0;
            }
        ...
        function reload_table() {
            ...
            ProductList.forEach((value, key) => {
                ...
                var unitQty = toIntQty(value.Quantity);
                ...
                total_amount = parseFloat(total_amount) + parseFloat(value.Total);
        ...
        function validateAndApplyDiscount() {
            ...
            ProductList.forEach(function(product) {
                var productAllowPercentage = 100;
